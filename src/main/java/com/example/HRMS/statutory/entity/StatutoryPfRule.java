...
/**
 * Versioned PF statutory rule values for a {@link StatutoryRuleVersionSet}
 * (table {@code statutory_pf_rule}; Phase 2 rule-value store).
 *
 * <p>Holds the structured PF rule values as {@code rulePayload} (JSON-as-TEXT)
 * plus normalized provenance/dimension columns (effective dates, authority,
 * source, verification metadata, release status). Concrete statutory constants
 * (rates, ceiling, EPS, wage-base) live only inside the payload as verified
 * release data — never hardcoded in Java. {@code DRAFT} rows may carry a
 * partial/empty payload; {@code VERIFIED}/{@code SUPERSEDED} rows are immutable
 * (enforced in the service layer). Maps the V19 table.
 */
@Entity
@Table(name = "statutory_pf_rule")
public class StatutoryPfRule {
